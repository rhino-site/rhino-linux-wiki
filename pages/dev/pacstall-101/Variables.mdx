...
<Callout type="error" emoji="☢⃝">
  **Caution:** In most cases, this should **not** be provided in a pacscript. Only use this for split packages (pacscripts that build multiple packages from the same source).
</Callout>
...
<Callout type="error" emoji="☢⃝">
  **Caution:** Use this _sparingly_, as any package using `epoch` cannot remove the `epoch` on the next `pkgver` update.
</Callout>
...
<Callout type="error" emoji="☢⃝">
  **Caution:** Do not include `priority` unless you know what you're attempting to do.
</Callout>
...
<Callout type="error" emoji="☢⃝">
  **Caution:** Do not use `mask` unless you know what you're doing.
</Callout>