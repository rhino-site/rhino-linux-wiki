...
> [!IMPORTANT]  
> The package suffix (`-deb`, `-git`, etc) must match the filename of the pacscript. If `pkgname="foo-deb"`, the file must be named `foo-deb.pacscript`. If there is no suffix, the file should be named `foo.pacscript`.
...
> [!CAUTION]
> In most cases, this should **not** be provided in a pacscript. Only use this for split packages (pacscripts that build multiple packages from the same source).
...
> [!CAUTION]  
> Use this _sparingly_, as any package using `epoch` cannot remove the `epoch` on the next `pkgver` update.
...
This is an array with URLs as elements. These will be downloaded by pacstall before the [prepare](#prepare) function. `source` can be created many different ways, and is incredibly powerful in functionality.
...
| Most specific  | `source_${distrorel}_$arch`  |
  > [!IMPORTANT]  
  > `-deb` packages can only use one source entry. Use the enhanced arrays to provide for multiple targets. For `-deb` packages, enhanced `source` and `*sums` arrays will override instead of append. This is not the case for the other enhanced arrays.
...
> [!CAUTION]
> Do not include `priority` unless you know what you're attempting to do.
...
> [!CAUTION]
> Do not use `mask` unless you know what you're doing.